    ...
    <div className="min-h-screen w-full">
      <header className="sticky top-0 z-20 backdrop-blur bg-white/80 border-b border-neutral-200">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={handlePrint} className="px-3 py-2 rounded-xl border border-neutral-300 bg-white hover:bg-neutral-100 text-sm">Print Current View</button>
            <button onClick={() => alert("Export coming soon")} className="px-3 py-2 rounded-xl border border-neutral-300 bg-white hover:bg-neutral-100 text-sm">Export (CSV/PDF)</button>
          </div>
        </div>
      </header>
      ...
      <section className="max-w-6xl mx-auto px-4 py-4 grid grid-cols-1 md:grid-cols-4 gap-3 print:hidden">
        <div className="md:col-span-1">
          ...
          <select value={grade} onChange={(e) => setGrade(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-xl border border-neutral-300 bg-white">
            {grades.map((g) => (
        ...
        <div className="md:col-span-2">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search code, description, strand…" className="w-full mt-1 px-3 py-2 rounded-xl border border-neutral-300 bg-white" />
        </div>
        <div className="md:col-span-1">
          <label className="text-xs font-medium text-neutral-600">Cross-Grade View</label>
          <select value={selectedCode || ""} onChange={(e) => setSelectedCode(e.target.value || null)} className="w-full mt-1 px-3 py-2 rounded-xl border border-neutral-300 bg-white">
            <option value="">— Select a standard to see progression —</option>
            {data.map((d) => (
              <option key={d.code + d.grade} value={d.code}>{d.code} ({d.grade})</option>
            ))}
          </select>
        </div>
      </section>
        ...
        <section className="max-w-6xl mx-auto px-4 pb-2">
          <div className="rounded-2xl border border-neutral-200 bg-white p-3">
            ...
            <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
              {progression.slice().sort((a, b) => a.grade.localeCompare(b.grade)).map((d) => (
      ...
      <main className="max-w-6xl mx-auto px-4 py-4 print:px-0">
        {Object.keys(groupedByStrand).length === 0 ? (
          <div className="text-neutral-600">No standards match your filters.</div>
        ) : (
                  ...
                  return (
                    <article key={d.code + d.grade} className={`rounded-2xl border ${color} p-4 shadow-sm`}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <button onClick={() => setExpanded((prev) => ({ ...prev, [d.code]: !prev[d.code] }))} className="px-3 py-1.5 rounded-xl border border-neutral-300 bg-white hover:bg-neutral-100 text-xs">{isOpen ? "Hide Details" : "Show Details"}</button>
                      </div>
